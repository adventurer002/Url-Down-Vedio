...
def test_parse_accepts_and_replays(client, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    r = c.post("/api/v1/videos/parse", json={"url": "https://public.example.com/v"})
    assert r.status_code == 202, r.text
    vid = r.json()["video_id"]
    r2 = c.get(f"/api/v1/videos/{vid}")
...
async def test_download_requires_ready(client, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    r = c.post("/api/v1/videos/parse", json={"url": "https://public.example.com/v"})
    vid = r.json()["video_id"]
    r2 = c.post("/api/v1/downloads", json={"video_id": vid, "format_id": "hd"})
    ...
    async with maker() as s:
        ...
        await s.commit()
    r3 = c.post("/api/v1/downloads", json={"video_id": vid, "format_id": "hd"})
    assert r3.status_code == 202, r3.text
    task_id = r3.json()["task_id"]
    r4 = c.get(f"/api/v1/downloads/{task_id}")
...
async def test_cancel_rules(client, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    r2 = c.post(f"/api/v1/downloads/{uploading_id}/cancel")
...
async def test_sse_replays_terminal_state(client) -> None:  # type: ignore[no-untyped-def]
    ...
    async with maker() as s:
            ...
            s.add(
                MediaFile(
                    owner_user_id=None, task_id=t.id, video_id=v.id, kind="video",
            ...
            await s.commit()
            task_id = str(t.id)
    with c.stream("GET", f"/api/v1/downloads/{task_id}/events") as r:
...
async def test_file_requires_completed(client) -> None:  # type: ignore[no-untyped-def]
    ...
    async with maker() as s:
            ...
            await s.commit()
            task_id = str(t.id)
    r = c.get(f"/api/v1/downloads/{task_id}/file")
